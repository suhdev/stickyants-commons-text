 ...
 * @since 1.0
 */
export class LevenshteinDistance implements EditDistance<number> {

    /**
     * Default instance.
     */
    static DEFAULT_INSTANCE = new LevenshteinDistance();
     ...
     */
    public LevenshteinDistance(threshold:number = null) {
     ...
     */
    public static getDefaultInstance():LevenshteinDistance {
     ...
     * @return result distance, or -1
     */
    private static limitedCompare(left:string,right:string,threshold:number) { // NOPMD
        ...
        let p = createArray(n+1,0); // 'previous' cost array, horizontally
        ...
        // iterates through t
        for (let j = 1; j <= m; j++) {
            let rightJ = right.charAt(j - 1); // jth character of right
            d[0] = j;
     ...
     */
    private static unlimitedCompare(left:string,right:string) {
        if (left == null || right == null) {
            throw new Error("CharSequences must not be null");
        }
