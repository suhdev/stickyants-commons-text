 ...
 * @since 1.0
 */
var LevenshteinDetailedDistance = /** @class */ (function () {
    function LevenshteinDetailedDistance() {
    }
    /**
     * <p>
     ...
     */
    LevenshteinDetailedDistance.prototype.LevenshteinDetailedDistance = function (threshold) {
     ...
     */
    LevenshteinDetailedDistance.prototype.apply = function (left, right) {
     ...
     */
    LevenshteinDetailedDistance.getDefaultInstance = function () {
     ...
     */
    LevenshteinDetailedDistance.prototype.getThreshold = function () {
     ...
     * @return result distance, or -1
     */
    LevenshteinDetailedDistance.limitedCompare = function (left, right, threshold) {
        ...
        if (n == 0) {
            return m <= threshold ? new LevenshteinResults_1.LevenshteinResults(m, m, 0, 0) : new LevenshteinResults_1.LevenshteinResults(-1, 0, 0, 0);
        }
        else if (m == 0) {
            return n <= threshold ? new LevenshteinResults_1.LevenshteinResults(n, 0, n, 0) : new LevenshteinResults_1.LevenshteinResults(-1, 0, 0, 0);
        }
        var swapped = false;
        if (n > m) {
        ...
        var p = util_1.createArray(n + 1, 0);
        var d = util_1.createArray(n + 1, 0); // cost array, horizontally
        var tempD; // placeholder to assist in swapping p and d
        var matrix = util_1.createArray(m + 1, function () { return util_1.createArray(n + 1, 0); });
        ...
        // iterates through t
        for (var j = 1; j <= m; j++) {
            var rightJ = right.charAt(j - 1); // jth character of right
            d[0] = j;
            // compute stripe indices, constrain to array size
            var min = Math.max(1, j - threshold);
            var max = j > Number.MAX_VALUE - threshold ? n : Math.min(n, j + threshold);
            // the stripe may lead off of the table if s and t are of different sizes
            if (min > max) {
                return new LevenshteinResults_1.LevenshteinResults(-1, 0, 0, 0);
            }
     ...
     */
    LevenshteinDetailedDistance.unlimitedCompare = function (left, right) {
        if (left == null || right == null) {
            throw new Error("CharSequences must not be null");
        }
        /*
        ...
        var m = right.length; // length of right
        if (n == 0) {
            return new LevenshteinResults_1.LevenshteinResults(m, m, 0, 0);
        }
        else if (m == 0) {
            return new LevenshteinResults_1.LevenshteinResults(n, 0, n, 0);
        }
        var swapped = false;
        if (n > m) {
        ...
        var p = util_1.createArray(n + 1, 0);
        var d = util_1.createArray(n + 1, 0);
        var tempD; //placeholder to assist in swapping p and d
        var matrix = util_1.createArray(m + 1, function () { return util_1.createArray(n + 1, 0); });
        // filling the first row and first column values in the matrix
        for (var index = 0; index <= n; index++) {
        ...
        var i; // iterates through left
        var j; // iterates through right
        var rightJ; // jth character of right
        var cost; // cost
        for (i = 0; i <= n; i++) {
            p[i] = i;
        }
     ...
     */
    LevenshteinDetailedDistance.findDetailedResults = function (left, right, matrix, swapped) {
        var delCount = 0;
        var addCount = 0;
        var subCount = 0;
        var rowIndex = right.length;
        var columnIndex = left.length;
        var dataAtLeft = 0;
        var dataAtTop = 0;
        var dataAtDiagonal = 0;
        var data = 0;
        var deleted = false;
        var added = false;
        ...
        return new LevenshteinResults_1.LevenshteinResults(addCount + delCount + subCount, addCount, delCount, subCount);
    };
    /**
     * Default instance.
     */
    LevenshteinDetailedDistance.DEFAULT_INSTANCE = new LevenshteinDetailedDistance();
    return LevenshteinDetailedDistance;
}());
exports.LevenshteinDetailedDistance = LevenshteinDetailedDistance;
